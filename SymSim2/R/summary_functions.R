...
#' plot umap of gene-expression data
#' @param umap_layout the layout calculated with the umap() function. The dimension is ncellsx2
#' @param meta the meta part of the observed_counts list 
#' @param label which label is used for coloring. usually this is pop or batch.
#' @export
plot_umap <- function(umap_layout, meta, label){
  label_set <- unique(meta[,label])
  col_set <-  rainbow(length(label_set))
  col_vec <- rep("black", dim(umap_layout)[1])
  for (isth in label_set){
    col_vec[which(meta[,label]==isth)] <- col_set[isth]
  }
  plot(x=umap_layout[,1], y=umap_layout[,2], pch=20, xlab="UMAP1", ylab="UMAP2",
       col=adjustcolor(col_vec, alpha.f = 0.6), main=sprintf("UMAP colored with %s",label))
  legend("topright",legend= paste(label, label_set), pch = 20, col = col_set)
}