...
GeneEffects <- function(ngenes,nregions,nevf,randseed,prob,geffect_mean,geffect_sd,evf_res,is_in_module){
  mod_strength <- 0.8
  set.seed(randseed)
  ...
  region_id <- lapply(c(1:nregions),function(i){ # region_id is like the gene_effect for the ATAC-Seq feature
    nonzero <- sample(size=nevf,x=c(0,1),prob=c((1-prob),prob),replace=T)
    nonzero[nonzero!=0]=rnorm(sum(nonzero),mean=geffect_mean,sd=geffect_sd)
    return(nonzero)
  })
  return(list(gene_effects=gene_effects, region_id=do.call(rbind,region_id)))
}
...
Get_params <- function(gene_effects,evf,match_param_den,bimod,scale_s, atacseq_data, region2gene, atac_effect){
  ...
  kon_atac <- atacseq_data %*% region2gene ; kon_atac1 <- kon_atac
  # nonzero_idx <- which(kon_atac>0)
  # nonzero_values <- kon_atac[nonzero_idx]
  # ranks <- rank(nonzero_values)
  # sorted <- sort(SampleDen(nsample=max(ranks),den_fun=match_param_den[[1]]))
  # scaled_params[[1]][nonzero_idx] <- sorted[ranks]
  ...
  if (length(zero_idx) < 1) {print("some problems")}
...
ContinuousEVF <- function(phyla,ncells,n_nd_evf,n_de_evf,n_de_evf_atac,impulse=F,evf_center=1,vary='s',
  ...
  N_DE_evfs <- c(N_DE_evfs, n_de_evf_atac )
  N_ND_evfs <- c(N_ND_evfs, n_de_evf+n_nd_evf-n_de_evf_atac )
  neutral <- SampleSubtree(root,0,evf_center,edges,ncells)
  param_names <- c("kon", "koff", "s", "atac")
  nparams <- length(param_names)
  evfs <- lapply(c(1:nparams),function(parami){
    ...
    if(N_DE_evfs[parami]!=0){
      ...
      if(impulse==T){
        ...
        de_evf <- lapply(c(1:N_DE_evfs[parami]),function(evf_i){
          ...
          re_order <- match(
            apply(neutral[,c(1:nparams)],1,function(X){paste0(X,collapse='_')}),
            apply(impulse[,c(1:nparams)],1,function(X){paste0(X,collapse='_')}))            
...
#' @import phytools
#' @export
SimulateTrueCounts <- function(ncells_total,min_popsize,i_minpop=1,ngenes, nregions,region2gene, atac_effect=1,
                               evf_center=1,evf_type="one.population",nevf=10,
                               n_de_evf=0,n_de_evf_atac =0, impulse=F,vary='s',Sigma=0.5,
                               phyla=NULL,geffect_mean=0,gene_effects_sd=1,gene_effect_prob=0.3, gene_module_prop=0,
  ...
  if(evf_type=='one.population'){
    ...
    evfs <- lapply(1:nparams, function(iparam){
    ...
    evf_res <- ContinuousEVF(phyla,ncells_total,n_nd_evf=nevf-n_de_evf,n_de_evf=n_de_evf,n_de_evf_atac = n_de_evf_atac,
  ...
  gene_effects_list <- GeneEffects(ngenes=ngenes,nregions=nregions, nevf=nevf,randseed=seed[2],prob=gene_effect_prob,
                              geffect_mean=geffect_mean,geffect_sd=gene_effects_sd,evf_res=evf_res,is_in_module=is_in_module)
  
  gene_effects <- gene_effects_list[[1]]
  region_id <- gene_effects_list[[2]]
  
  # first generate scATAC-Seq data, only after this we can generate scRNA-Seq data
  params_region <- evf_res[[1]][[3]] %*% t(region_id) # use the EVF for s, as s had DiffEVFs
  # now rescale these values to a distribution of real scATAC-Seq dataset
  #load("SymSim2/data/eg_atac_seq.RData")
  #atacseq_den <- density(as.matrix(eg_atac_seq),n=5000)
  load("SymSim2/data/dens_nonzero.RData") # this is the density function of log(x+1), where x is the non-zero values
  p_zero <- 0.8 # the proportion of 0s we see in the data is 90%, but to account for technical noise (approx. 50% of the material is lost), we set p_zero as 0.5 here.
  ...
  #ssorted <- sort(SampleDen(nsample=max(ranks),den_fun=atacseq_den))
  ...
  if(!is.null(param_realdata)){
    ...
    params <- Get_params(gene_effects,evf_res[[1]],match_params_den,bimod,scale_s=scale_s,
                         atacseq_data=atacseq_data, region2gene=region2gene, atac_effect = atac_effect)
  }else{
  ...
  return(list(counts=counts,atac_seq=t(atacseq_data), gene_effects=gene_effects,
              cell_meta=cell_meta,kinetic_params=params, in_module=is_in_module))
}
...
#' @export
DivideBatches <- function(observed_counts_res, atacseq_noisy=NA, nbatch, batch_effect_size=1){
  ...
  if (!all(is.na(atacseq_noisy))){
    ...
    temp <- lapply(1:nregions, function(iregion) {
      return(runif(nbatch, min = region_mean[iregion]-batch_effect_size, max = region_mean[iregion]+batch_effect_size))
    })
    mean_matrix <- do.call(rbind, temp)
    ...
    return(list(scRNAseq=observed_counts, scATACseq=atacseq_noisy, cell_meta=meta_cell))
  } else {
